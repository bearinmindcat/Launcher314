...
### Drawer Search
Added a "Fuzzy app search" toggle — find apps by initials (e.g. "yt" → YouTube).  
Added a "Recency search bias" toggle — while searching, apps you've opened recently rank higher.  
...
### Folders
Folders can now show a single chosen image as their icon instead of the 2×2 app-contents preview.  
...
### Performance & Fixes
Added an app-list cache to the drawer to fix the occasional "spin of death" / freeze on open.  
Moved shaped-icon generation off the main thread to smooth out high-refresh-rate devices.  
